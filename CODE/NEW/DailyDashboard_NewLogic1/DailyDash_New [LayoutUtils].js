...
const DASHBOARD_COLORS = {
  ...
  function setDashboardColumnWidths(sheet) {
    ...
    sheet.setColumnWidth(1, 200); // A - First column
    sheet.setColumnWidth(2, 20);  // B - Spacer
    sheet.setColumnWidth(3, 200); // C - Second column
    sheet.setColumnWidth(4, 20);  // D - Spacer
    sheet.setColumnWidth(5, 200); // E - Third column
    sheet.setColumnWidth(6, 20);  // F - Spacer
    sheet.setColumnWidth(7, 200); // G - Fourth column
    sheet.setColumnWidth(8, 20);  // H - Spacer
    sheet.setColumnWidth(9, 200); // I - Fifth column
  }
  ...
  function setKpiRowHeights(sheet, startRow) {
    ...
    sheet.setRowHeight(startRow + 1, 40); // Main value row - taller for large text
    sheet.setRowHeight(startRow + 2, 25); // Change indicator row
    sheet.setRowHeight(startRow + 3, 25); // Subtitle row
  }
  ...
  /**
   * Formats a change indicator for a KPI tile
   * @param {Sheet} sheet - The Google Sheet
   * @param {number} row - The row for the change indicator
   * @param {number} column - The column for the change indicator
   ...
   */
  function formatKpiChange(sheet, row, column, change, reverseColors = false) {
    if (change === 0) return;
  ...
  function testLayoutUtils() {
    ...
    formatKpiValue(sheet, startRow + 1, 1, 42);
    formatKpiChange(sheet, startRow + 2, 1, 5);
