...
function updateDailyDashboardTest() {
  ...
  try {
    ...
    // Clear ALL existing content from after the Rep table to end of potential low ratings section
    // This ensures no duplicate content remains from previous runs
    const clearStartRow = RepTableEndRow + 1; // Start clearing from the row after the Rep table ends
    dash.getRange(`B${clearStartRow}:O40`).clearContent();
    try {
      dash.getRange(`B${clearStartRow}:O40`).breakApart();
      } catch (e) {
